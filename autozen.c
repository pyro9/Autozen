...
#define PULSEAUDIO_MODE 1
#define OSS_MODE 0
char audio_mode = PULSEAUDIO_MODE;
int dsp;
...
volatile int mode = TS_MODE_BITONAL;
...
int SetSampleFmt(int dsp, unsigned int fmt)
{
	...
	if (ioctl(dsp, SNDCTL_DSP_SETFMT, &tmp)==-1) { /* Fatal error */
...
#ifdef USE_PULSE
	if(bUsePulse) {
		pa = pa_simple_new(NULL, "AutoZen", PA_STREAM_PLAYBACK, NULL, "playback", &pa_ss, NULL, NULL, &error);
		if(!pa) {
			fprintf(stderr, __FILE__": pa_simple_new() failed: %s\n", pa_strerror(error));
		} else {
			audio_mode = PULSEAUDIO_MODE; 
			latency = pa_simple_get_latency(pa, &error);
			if(latency<0)
				fprintf(stderr, __FILE__": pa_simple_get_latency() failed: %s\n", pa_strerror(error));

			fprintf(stderr, "Using PulseAudio (latency = %d, error=%lld)\n", latency, error);
			return SampleRate;
		}
	}
#endif
...
//		return(0);
	...
	SetSampleFmt(dsp, AFMT_S16_NE);

	fprintf(stderr, "Using OSS (/dev/dsp)\n");
	return SampleRate;
}

int PaSampleOut(char *left, char *right) {
	static short buffer[100];
	static int count=0;
	int error;
#ifdef USE_PULSE
	memcpy(&(buffer[count]), left, sizeof(short));
	memcpy(&(buffer[count+1]), right, sizeof(short));
	count+=2;
	
	if(count>=50) {
		if (pa_simple_write(pa, buffer, sizeof(short)*count, &error) < 0) {
...
#ifdef USE_PULSE
int SampleOut(int left, int right) {
	int error;
	if(audio_mode == PULSEAUDIO_MODE) {
		PaSampleOut(&left, &right);
	} else {
		write(dsp,&left,SAMPLE_SIZE);	// left
		write(dsp,&right,SAMPLE_SIZE);	// right!
	}
}
#else
int SampleOut(int left, int right) {
...
void *SoundThread(void *v)
{
	...
	SampleRate = OpenSound();

	InitWavegen(SampleRate);
	
	while(!bQuit) {
		for(j=0;j<SAMPLE_RATE;j++) {
//			TimeStep(samples, increment, detune, nHarmonics, volume, TS_MODE_CLASSIC);
			TimeStep(samples, increment, detune, nHarmonics, volume, mode);
			SampleOut(samples[0], samples[1]);
...
GtkWidget *create_HelpWindow (const char *text)
{
	...
	tb = gtk_text_view_get_buffer (GTK_TEXT_VIEW (help_text));
...
//	gtk_widget_show (lable);
	...
	// LiLa Logo
	LiLaPix = gdk_pixmap_create_from_xpm_d( window->window,  &LiLaBit, &(wstyle->bg[GTK_STATE_NORMAL]), (gchar **) &logo );
...
int main (int argc, char *argv[])
{
	...
	while(next_arg < argc && argv[next_arg][0] == '-') {
		...
		if(!strcasecmp(argv[next_arg], "-colorbox")) {
			...
			printf("%s [-colorbox [<x> <y>]] [-harmonics <n>] [-oss] [<sequence>]\n",argv[0]);
			exit(0);
		} else if(!strcasecmp(argv[next_arg], "-oss") || !strcasecmp(argv[next_arg], "-o")) {
			bUsePulse=0;
			next_arg++;
		}
